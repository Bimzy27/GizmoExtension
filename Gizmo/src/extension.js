...
function activate(context) {
    ...
    const config = vscode.workspace.getConfiguration('Gizmo');
    const dllPath = config.get('compDllPath');
    ...
    const disposable = vscode.commands.registerCommand('Gizmo.helloWorld', () => {
        ...
        if (isString(dllPath)) {
            // Load the DLL using Node.js's `require` function
            const dll = require(dllPath);
            // Access the compile method
            const compile = dll.compile;
            // Example usage:
            const codeToCompile = "your code here";
            compile(codeToCompile);
            // Handle the compilation result
            vscode.window.showInformationMessage('Compile Success!');
        }
        else {
            console.error("DLL path is not a string.");
        }
    });
